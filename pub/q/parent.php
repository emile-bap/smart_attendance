...
<head>
    ...
    <link rel="shortcut icon" href="<?= @$logo; ?>">
<title><?= @$abbreviation; ?> <?php if($_SESSION['position_name']=='Student'){ echo 'Parent '; } else{} ?></title>
...
<hr><h4><?php if($_SESSION['position_name']=='Student'){ echo 'Parent '; } else{} ?> Dashboard</h4>
</center>
...
if(isset($_GET['key'])) {
    ...
    cl.class_id=pr.class_id WHERE pr.username='".$_SESSION['username']."' ORDER BY ps.abbreviation, pp.position_id ASC LIMIT $limit OFFSET $offset");
    try {
    $stms_access->execute(array());
    $row_count_access = $stms_access->rowCount();
    if ($row_count_access > 0) {
    ...
    ps ON ps.id = pr.school_id INNER JOIN 
    centers AS c ON c.center_id=pr.center INNER JOIN classes AS cl ON
    cl.class_id=pr.class_id WHERE pr.username='".$_SESSION['username']."'");
    $stmtf->execute();
    $result = $stmtf->fetch(PDO::FETCH_ASSOC);
    $total_pages = ceil($result['total'] / $limit);
    $count=1; ?>
    ...
    <th style="color:#fff; background-color:#005292; text-align:center;">Option</th>
    </tr></thead><tbody>
    <?php $i=1;
    while($access = $stms_access->fetch(PDO::FETCH_ASSOC)) {
...
Attendance: <b><a href="print?reportofattendancesingle=<?= $record_1; ?>&nnc=<?= $record_22; ?>&title=Attendance report of <?= $record_22; ?> class today <?= date('Y-m-d-H:i:s'); ?>"> View attendance of <?=  $record_22; ?></a></b>
...
<input type="text" class="form-control" name="data_1" value="<?= $data_1; ?>" required readonly></div>
<div style="float:right; width:49%;">Other name<br>
<input type="text" class="form-control" name="datax_1" value="<?= $data_2; ?>" required readonly></div>
<div style="float:left; width:49%;">
Gender<br />
...
<div style="float:left; width:49%;">
Email<br>
<input type="text" readonly class="form-control" name="data_2" value="<?= $data_3; ?>" required></div>
...
<input type="text" readonly class="form-control" name="father" value="<?= $father; ?>" placeholder="Father..." required></div>
<div style="float:right; width:49%;">Mother<br>
<input type="text" readonly class="form-control" name="mother" value="<?= $mother; ?>" placeholder="Mother..." required></div>


<div style="float:left; width:49%;">Username<br>
<input type="text" readonly class="form-control" name="data_5" value="<?= $data_6; ?>" required></div>
...
$save_staffN= $DB_con->prepare("INSERT INTO registration ( user_othername, user_name,user_email, user_phone, user_gender, user_photo, user_country, user_cell, user_position, username, user_password, reg_time_date, user_status, school_id, center, user_thingerprint, class_id, parent) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
$save_staffN->execute(array( $last_name, $first_name, $user_email, $user_phone, $gender, $picture, $country, $cell_id, $position, $username, $password, $time_date, $status, $school_id, $center, $fingerprints, $class_ids, $parent));
 ?>
